...
async function runScraper() {
    ...
    try {
        ...
        for (let paginaAtual = 1; paginaAtual <= TOTAL_PAGINAS; paginaAtual++) {
            ...
            const linksDaPagina = await page.$$eval("a", (elements) => elements
            ...
            try {
                ...
                const dadosRaw = await pageDetalhe.evaluate(() => {
                    const linhas = document.body.innerText
                        ...
                        .map((l) => l.trim())
                        .filter((l) => l.length > 0);
                    const addressLine = linhas.find((line) => line.includes("João Monlevade") &&
                        line.includes("-") &&
                        !line.includes("R$") &&
                        !line.toUpperCase().includes("ALUGUEL") &&
                        !line.toUpperCase().includes("VENDA")) || "";
                    const externalIdLine = linhas.find((line) => line.toUpperCase().includes("COD:")) || "";
                    const priceLine = linhas.find((line) => line.includes("R$") && line.includes(",")) ||
                        "0";
                    const idxCod = linhas.findIndex((line) => line.toUpperCase().includes("COD:"));
                    let title = "Imóvel";
                    if (idxCod >= 0 && linhas.length > idxCod + 1)
                        title = linhas[idxCod + 1];
                    const linksFotos = Array.from(document.querySelectorAll('a[rel*="shadowbox"], a[rel*="galeria"]'))
                        .map((a) => a.href)
                        .filter((href) => href && (href.includes(".jpg") || href.includes(".png")));
                    const caracteristicas = { quartos: 0, suites: 0, vagas: 0 };
                    const boxes = document.querySelectorAll('div[style*="border: 1px solid #ccc"]');
                    boxes.forEach((box) => {
                        const label = box.querySelector("span")?.textContent?.trim().toUpperCase() ||
                            "";
                ...
                let precoFinal = 1000;
                if (matchPreco) {
                    precoFinal = Number(matchPreco[1].replace(/\./g, "").replace(",", "."));
                }
                ...
                if (titleUpper.includes("APARTAMENTO") || titleUpper.includes("APT"))
                    ...
                    propertyType = "LOTE";
                else if (titleUpper.includes("COMERCIAL") ||
                    titleUpper.includes("SALA"))
                    propertyType = "COMERCIAL";
                else if (titleUpper.includes("REPUBLICA"))
                ...
                if (dadosRaw.externalId) {
                    ...
                    console.log(`✅ ${dadosRaw.externalId} | Bairro: ${bairro} | Fotos: ${dadosRaw.galeria.length}`);
                }
            }
            catch (err) {
