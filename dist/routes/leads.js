...
export async function leadEventsRoutes(app) {
    app.post("/lead-events", async (request, reply) => {
        ...
        const { userId, userName, userEmail, propertyId, eventType } = bodySchema.parse(request.body);
    ...
    app.get("/admin/lead-events/top-properties", async (_request, reply) => {
        ...
        const result = properties
            .map((property) => {
            const interestClicks = property.leadEvents.filter((event) => event.eventType === "interest_click");
            const whatsappClicks = property.leadEvents.filter((event) => event.eventType === "whatsapp_click");
            const uniqueInterestedUsers = new Set(interestClicks.map((event) => event.userId)).size;
            const uniqueWhatsappUsers = new Set(whatsappClicks.map((event) => event.userId)).size;
            const conversionRate = interestClicks.length > 0
                ? Number(((whatsappClicks.length / interestClicks.length) * 100).toFixed(1))
                : 0;
            return {
                id: property.id,
    ...
    app.get("/admin/lead-events/property/:propertyId/users", async (request, reply) => {
        ...
        return reply.send({
            property,
            users: Array.from(usersMap.values()).sort((a, b) => b.lastEventAt.getTime() - a.lastEventAt.getTime()),
        });
    });
    ...
    app.get("/admin/lead-events/summary", async (_request, reply) => {
        ...
        const interestClicks = allEvents.filter((event) => event.eventType === "interest_click").length;
        const whatsappClicks = allEvents.filter((event) => event.eventType === "whatsapp_click").length;
        ...
        const uniqueProperties = new Set(allEvents.map((event) => event.propertyId))
            .size;
        const conversionRate = interestClicks > 0
